...
export default function StudentsTab() {
  ...
  // Safely extract arrays with fallback checks
  const studentClasses = Array.isArray(studentClassesData?.mysqlResult) 
    ? studentClassesData.mysqlResult 
    : (Array.isArray(studentClassesData) ? studentClassesData : []);

  const users = Array.isArray(usersData?.mysqlResult) 
  ...
  const students = studentClasses.map(sc => {
    ...
    return {
      ...
      email: userInfo?.email_address || userInfo?.email || 'No email'
    };
  });
